...
But first, let's get our app deployed on Osiris. This is actually a critical step.  We need to ensure that our app will run on Osiris **before** we start making significant changes.
...
* Then type `git commit -m "Ready for Osiris!"` to commit all local changes.
* Push those changes to GitHub.
* Log in to the [Osiris Orchestrator](https://deploy.osiris.cyber.nyu.edu/) and deploy the application in the same fashion that we did earlier in this CHIP (see [here](part_0_create_saas_app.md#deploy-to-osiris)).
* Once deployed, verify that the Osiris-deployed Wordguesser behaves the same as your development version before continuing.
